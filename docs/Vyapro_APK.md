...
`https://github.com/avi-watwani/portfolio-vite/releases/download/vyapro-v1.0.1/vyapro.apk`
...
```bash
gh release create vyapro-v1.0.2 \
  /path/to/your-build.apk#vyapro.apk \
  --repo avi-watwani/portfolio-vite \
  --title "Vyapro Android APK v1.0.2" \
  --notes "Android APK for Vyapro direct install."
```

Bump the tag / title for each release (`vyapro-v1.0.3`, etc.), then point `VYAPRO_APK_URL` at that tag.
